...
public class BossRaditzAI : MonoBehaviour
{
    [Header("AI Settings")]
    ...
    void MakeDecision()
    {
        ...
        if (distance <= 1.8f && skillInput.CanUseSkill("H"))
        {
            UpdateFacingDirectionForSkill();
            skillInput.TriggerSkill("H"); // Chiêu đấm
        }
        else if (distance <= 4.3f && skillInput.CanUseSkill("J"))
        {
            UpdateFacingDirectionForSkill();
            skillInput.TriggerSkill("J"); // Chiêu chưởng
        }
        else if (distance <= 5f && skillInput.CanUseSkill("K"))
        {
            UpdateFacingDirectionForSkill();
            skillInput.TriggerSkill("K"); // Chiêu teleport
        }
        else if (distance <= 2.2f && skillInput.CanUseSkill("L"))
        {
            UpdateFacingDirectionForSkill();
            skillInput.TriggerSkill("L"); // Chiêu tự phát nổ
        }
    }
    ...
    // Tìm target gần nhất trong phạm vi 'detectionRange'
